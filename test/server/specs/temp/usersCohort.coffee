...
stories = ->
  ...
  IT 'Create account and update lots of things', ->
    _id = null
    ...
    expectCohort = (session, expCompare, isAuthor, log, cb) ->
      ...
      DB.docById 'User', _id, (user) ->
        if !FIXTURE.users[signupData.userKey]
          FIXTURE.users[signupData.userKey] = user
    ...
    signup = (cb) ->
      # $log('STUB.spy', mailman.sendTemplate)
      spy1 = STUB.spy(mailman,'sendTemplate')
      PAGE firstRequestUrl, null, (err, resp) ->
        # firstSessionId = anonSesh.sessionID
        SUBMIT "/auth/signup", signupData, (sResp) ->
          _id = sResp._id
          expectCohort(sResp)
          # hash = spy.args[0][1]
    ...
    updateAccount = (cb) ->
      PUT "/users/me/username", { username: signupData.userKey }, {}, (s4) ->
        ...
        PUT "/users/me/initials", { initials: 'jk' }, {}, (s5) ->
          PUT "/users/me/location", FIXTURE.wrappers.localization_melbourne.locationData, {}, (s6) ->
            PUT "/users/me/bio", { bio: 'a flow bio'}, {}, (s7) ->
              expectCohort(s7)
              # SETUP.connectOAuth s7, 'github', FIXTURE.users.ape1.social.gh, (s8) ->
                # expectCohort(s8)
                # SETUP.connectGoogle s8, FIXTURE.users.ape1.google, (s9) ->
              cb()

    applyToBeAnExpert = (cb) ->
      d = rate: 80, breif: 'yo flow', tags: [FIXTURE.tags.angular]
      POST "/experts/me", d, (expert) ->
        expert = expert
        PAGE "/logout",  (err, resp) ->
          global.cookie = null
          SUBMIT "/auth/login", signupData, (s10) ->
            # $log(sResp)
