...
describe " HUMAN".spec, ->
  ...
  IT "[501] Not Impl", ->
    opts = {status:501,contentType:/text/}
    PAGE '/apple-app-site-association', opts, (txt1) =>
      PAGE '/browserconfig.xml', opts, (txt2) =>
        PAGE '/.well-known/dnt-policy.txt', opts, (txt3) =>
          PAGE '/rules.abe', opts, (txt4) =>
            DONE()

  # IT '/post/thumb/55c02b22d131551100f1f0da', -> PAGE @test.title, {status:301,contentType:/text/}, (txt) -> DONE()
  # IT '/favicon.ico', -> IMG @
...
describe " SEARCH BOT".spec, ->
  ...
  IT '/post/thumb/55c02b22d131551100f1f0da', -> PAGE @test.title, assign({status:301},@optsExpect), (txt) -> DONE()
  ...
  it "[200]OK for all sitemap.xml loc urls"


describe " BAD BOT".spec, ->

  beforeEach ->
    @optsExpect = contentType: /text/, status:200, ua: FIXTURE.http.UA.lib.jsoup


  IT '/post/thumb/55c02b22d131551100f1f0da', -> PAGE @test.title, @optsExpect, (txt) -> DONE()
