...
DESCRIBE "Util", ->
  ...
  IT "Can get multiTime", ->
    ...
    expect(multitime.indexOf("Sat 25 00:00 UTC | Fri 24 5PM PDT | Fri 24 7PM CDT")).to.equal(0)
    bOld = FIXTURE.clone('bookings.swap1')
    bOld.datetime = ISODate("2015-03-12T03:15:18.576Z")
    multitime2 = BookingUtil.multitime(bOld)
    expect(multitime2.indexOf("Thu 12 03:15 UTC")).to.equal(0)
    DONE()
  ...
  IT "Purpose for pending status", ->
    ...
    expect(b.participants[0].info.name.indexOf("Morgan")).to.equal(0)
    expect(b.participants[0].timeZoneId).to.equal("America/Los_Angeles")
    expect(b.participants[1].info.name.indexOf("Billy")).to.equal(0)
    expect(b.participants[1].timeZoneId).to.equal("America/Chicago")
    {name,purpose} = BookingUtil.chatGroup(b)
    expect(name.indexOf("morgan-billy")).to.equal(0)
    # During Daylight Savings
    purpose = purpose.replace /ST,/g, "DT,"
    expect(purpose.indexOf("http://booking.airpa.ir/558aa2454be238d1956cb8aa Morgan (PDT, San Francisco, CA, USA) + Billy (CDT, Houston, TX, USA). WAITING to confirm 90 mins @ Sat 25 00:00 UTC | Fri 24 5PM PDT | Fri 24 7PM CDT")).to.equal(0)
    DONE()
  ...
  IT "Purpose for confirmed status", ->
    ...
    # During Daylight Savings
    purpose = purpose.replace /ST,/g, "DT,"
    expect(purpose.indexOf("http://booking.airpa.ir/558aa2454be238d1956cb8aa Morgan (PDT, San Francisco, CA, USA) + Billy (CDT, Houston, TX, USA). CONFIRMED 90 mins @ Sat 25 00:00 UTC | Fri 24 5PM PDT | Fri 24 7PM CDT")).to.equal(0)
    DONE()
  ...
  IT "Purpose for followup status", ->
    ...
    {name,purpose} = BookingUtil.chatGroup(b)
    expect(name.indexOf("michael-jj")).to.equal(0)
    # During Daylight Savings
    purpose = purpose.replace /ST,/g, "DT,"
    expect(purpose.indexOf("http://booking.airpa.ir/559dc6ff476dc61100a02069 Michael (PDT, San Francisco, CA, USA) + JJ (EDT, New York, NY, USA). FEEDBACK required to payout expert for 60 mins on Wed 25 11:30 UTC | 4:30AM PDT | 7:30AM EDT")).to.equal(0)
    DONE()
  ...
  IT "Purpose for old no timeZoneId bookings", ->
    ...
    # During Daylight Savings
    purpose = purpose.replace /ST,/g, "DT,"
    expect(purpose.indexOf("http://booking.airpa.ir/54dc2d2fd137810a00f2813b Daniel + Adam. FEEDBACK required to payout expert for 60 mins on Thu 12 03:30 UTC")).to.equal(0)
    DONE()
...
#   #         expect.indexOf((e.message,"Booking [#{b1._id}] must be in folloup or complete state").to.equal(0)
...
#   #                 expect.indexOf((e2.message,"Booking [#{b1._id}] must be in folloup or complete state").to.equal(0)
#   #                 LOGIN {key:"admin"}, ->
...
#   #                         expect.indexOf((e3.message,"Booking [#{b1._id}] must be in folloup or complete state").to.equal(0)
#   #                         DONE()
...
#   #     PUT "/bookings/#{b1._id}/#{b1.expertId}/customer-feedback", {}, {status:403}, (e) ->
#   #       expect.indexOf((e.message,"Booking customer feedback review required").to.equal(0)
#   #       rev1 = { questions: [
...
#   #           LOGIN {key:'dros'}, ->
#   #             PUT "/bookings/#{b1._id}/#{b1.expertId}/customer-feedback", {}, {status:403}, (e) ->
#   #               expect.indexOf((e.message,"Not a customer on booking").to.equal(0)
#   #               DONE()
...
#   #           expect.indexOf((err.message, "Payout[#{booking1.orderId}] must be released by owner").to.equal(0)
