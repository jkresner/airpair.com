var cfg                      = null
var FnUtil                   = require('honeycombjs').Util.Function
...
var retryableFns = {
  getScopes(user, cb) {
    ...
    this.api.user.get(payload, (e, r) => {
      if (e && /Bad credentials/.test(e.message))
  ...
  getPullRequests(user, repo, opts, cb) {
    opts = opts || {}
    var owner = opts.owner || cfg.org
    ...
    this.api.repos.getContent(payload, (e, r) => {
      ...
      if (e && e.code == 404)
      {
        ...
        this.api.repos.get({ user: repoOwner, repo }, (err, response) => {
          ...
          else if (err.code === 404 && repoOwner == cfg.org)
            return cb(Error(`No repo at ${repoOwner}/${repo}... Seems it was deleted?`))
  ...
  addContributor(user, org, repo, cb) {
    var payload = { user: org || cfg.org, repo }
    this.api.repos.fork(payload, (e,r) => {
      cb(e, r, payload)
    })
  },
}
...
Object.keys(retryableFns).forEach(function(fnName) {
  wrapper[fnName] = function() {
    ...
    LOG('wrpr.ghcall', `gh.${fnName}:${retries}`, by)
    var attempt = hyjackCallback(retryableFns[fnName], done => (e,r,payload) => {
      ...
      if (e) {
        ...
        if (this.retrying > 0 && err.match(retryError)) {
          LOG('wrpr.ghretry', `gh.${fnName} retrying(this.retrying) in ${cfg.retry.delay}ms`, e)
...
var Step = (wrpr, fullRepoName, errorCB) => function() {
  ...
  args.push((e,r,payload) => {
    ...
    if (e && e.constructor !== String) {
      ...
      LOG('wrpr.ghstep','step.fail     '.red, opLog.red, e.message.white, payload)
      return errorCB(e)
    }
    else if (e) LOG('wrpr.ghstep','step.unsmooth', opLog, e, JSON.stringify(payload))
    else LOG('wrpr.ghstep','step.success', opLog)
    cb(e, r)
  })
  LOG('wrpr.ghstep','step.begin    ', opLog)
  op.apply(wrpr, args)
}
